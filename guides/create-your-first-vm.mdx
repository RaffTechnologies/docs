...
VPC membership is **optional**. A VM with no VPC is still fully reachable via its public IP — you only join a VPC when two or more VMs need to talk to each other privately.

### No VPC (Default)

If you don't pass any VPC options, the VM is created **without a VPC**. It has its public IP only; it can't reach other VMs over a private subnet.
