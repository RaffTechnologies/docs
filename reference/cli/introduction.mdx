...
The JSON shape matches the [public API response schema](/api-reference/introduction). Anything you can do with `curl` you can do with `raff <command> --output json`.
...
The CLI tracks the public API surface. Anything documented in the [API Reference](/api-reference/introduction) is available through the CLI with one exception: low-traffic admin endpoints aren't exposed (admin-service has its own internal tooling). For the customer-facing surface — VMs, projects, VPCs, IPs, firewalls — there's full parity.
