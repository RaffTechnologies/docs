...
Delete multiple VMs in one call. By default, attached volumes detach (preserved), and shared VPCs are kept if other VMs still use them. Pass `--keep-volumes false` to also delete volumes; `--keep-vpcs false` to also delete VPCs (subject to the shared-VPC safety rule — see [Delete a VM](/products/build/virtual-machines/quickstart-guides/delete-vm)).
