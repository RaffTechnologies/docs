...
```bash
raff kubernetes kubeconfig <cluster-id> [--save <path>] [--ttl <duration>]
raff kubernetes kubeconfig rotate <cluster-id>
```

Prints the admin kubeconfig to stdout, or writes it to a file (mode 0600) with `--save`. `--ttl` issues a **short-lived** kubeconfig instead (`--ttl 1h`, `10m`–`720h`) — it expires on its own, so it is safer for teammates and CI. `kubeconfig rotate` invalidates every previously issued short-lived kubeconfig immediately; the admin kubeconfig is unaffected.
