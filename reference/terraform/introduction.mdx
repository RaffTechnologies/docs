...
More resources land as the public API expands. For features not yet in the provider, drop down to the [public API](/api-reference/introduction) directly via Terraform's `http` data source or a `null_resource` with `local-exec`.
