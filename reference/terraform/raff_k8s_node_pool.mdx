...
Manages an **additional node pool** on a Raff Kubernetes cluster. The cluster's first pool is defined on [`raff_k8s_cluster`](/reference/terraform/raff_k8s_cluster) (`default_pool`). Each pool has its own `plan_id`, so pools in one cluster can use different plans and tiers.
...
| `plan_id` | int | Yes | Worker node plan ID for this pool — pools may use different plans |
| `node_count` | int | Yes | Node count (1–20; the cluster keeps ≥2 workers overall) |
