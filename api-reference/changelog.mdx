...
- 🟢 **Added** — **Self-renewing kubeconfig in the CLI** — `raff kubernetes kubeconfig <cluster-id>` writes a kubeconfig that carries no credential and fetches a short-lived token for each `kubectl` command (same exec-credential mechanism as `gcloud` and `aws eks`). Nothing long-lived is stored on disk; `--static` keeps the old file-with-a-token shape for environments that cannot run the CLI
