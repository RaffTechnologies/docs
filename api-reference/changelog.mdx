...
**Virtual Machines — Console access policy**

- 🟢 **Added** — **Per-VM console 2FA requirement** — `PATCH /api/v1/vms/{id}/console-policy` with `{"console_require_mfa": true}` makes the VM's web console (terminal and VNC) ask for a fresh authenticator code before it opens (one confirmation unlocks protected consoles for 1 hour). Off by default; the VM owner switches it on from **VM → Security**. API keys cannot open a console on such a VM. `GET /api/v1/vms/{id}` now returns `console_require_mfa`. CLI: `raff vm console-policy <vm-id> --require-mfa`; Terraform: `console_require_mfa` on `raff_vm`

### September 7, 2026

**Pricing — VMs, Kubernetes, Managed Databases**

- 🔵 **Updated** — **Standard (General Purpose) VM plans repriced** — [List VM Pricing](/api-reference/catalog/list-vm-pricing) now returns the new monthly rates for every shared-CPU size above 1 vCPU / 1 GiB (2 vCPU / 4 GiB is \$17.99, 4 vCPU / 8 GiB \$33.99, 8 vCPU / 16 GiB \$71.99, 16 vCPU / 32 GiB \$139.99). The 1 vCPU / 1 GiB (\$4.99) and 2 vCPU / 8 GiB (\$27.99) sizes are unchanged. Bundled SSD is 20 GB per GiB of RAM up to 8 GiB, +10 GB per GiB above that, capped at 320 GB
- 🔵 **Updated** — **Premium (CPU-Optimized) VM plans repriced and redefined** — every vCPU now has half a physical thread reserved (guaranteed), with burst to the full thread. New rates run \$15.99 (1 vCPU / 1 GiB) to \$288.99 (16 vCPU / 32 GiB); SSD allotments are unchanged
- 🔵 **Updated** — **Windows VM plans repriced** — \$13.99 (1 vCPU / 2 GiB) to \$174.99 (16 vCPU / 32 GiB). The Windows Server licence is still a separate \$15/mo line
- 🔵 **Updated** — **Kubernetes worker plans** — `GET /api/v1/k8s/node-plans` returns the new rates: K8s Standard \$17.99, Performance \$33.99, High Memory \$71.99, Large \$119.99; CO Standard \$36.99, CO Performance \$72.99, CO High Memory \$144.99, CO Large \$192.99, CO Scale \$288.99. Starter (\$9.99), Scale (\$229.99) and the \$30/mo HA control plane are unchanged. PVC storage is now \$0.10/GB/month (was \$0.08)
- 🔵 **Updated** — **Managed Database plans** — `GET /api/v1/database/plans` returns the new Postgres / MySQL ladder from \$22.99 (1 vCPU / 2 GiB) to \$367.99 (16 vCPU / 32 GiB) and the matching Valkey sizes (2 GB \$22.99 to 32 GB \$293.99); the \$7.99 entry size, Free tier, ClickHouse and Kafka ladders are unchanged. Extra storage is now \$0.15/GB-month (was \$0.12). HA and read-replica multipliers are unchanged
- Volumes, backups, snapshots, reserved IPs, Object Storage, Raff Apps and Functions prices are unchanged

### August 28, 2026

**Managed Databases — Plans**

- 🔴 **Removed** — **Sub-4 GiB ClickHouse and Kafka plans** — `GET /api/v1/database/plans` no longer returns the 1 vCPU / 1 GiB, 1 vCPU / 2 GiB and 2 vCPU / 2 GiB tiers for `clickhouse` and `kafka`; both ladders now start at 2 vCPU / 4 GiB (ClickHouse $59.99/mo, Kafka $79.99/mo). Neither engine runs reliably below 4 GiB — ClickHouse's own sizing guidance is that under 4 GB is not recommended, and a Kafka broker needs headroom for page cache on top of its heap. No existing instances were affected. Postgres, MySQL and Valkey ladders are unchanged and still start at $7.99

### August 27, 2026

**Kubernetes — API**

- 🔵 **Updated** — **Every kubeconfig now expires.** `GET /api/v1/k8s/clusters/{cluster_id}/kubeconfig` returns a kubeconfig valid for **7 days** by default (`expires_in` still selects 10 minutes–30 days); it no longer returns a permanent admin credential, which could never be revoked. [Rotate access](/api-reference/kubernetes/rotate-kubeconfig-access) now invalidates every credential a customer holds
- 🟢 **Added** — **Self-renewing kubeconfig in the CLI** — `raff kubernetes kubeconfig <cluster-id>` writes a kubeconfig that carries no credential and fetches a short-lived token for each `kubectl` command. Nothing long-lived is stored on disk; `--static` keeps the old file-with-a-token shape for environments that cannot run the CLI
- 🟢 **Added** — **Per-pool node plans** — every node pool now has its own `plan_id`: create clusters with mixed pools (`node_pools[].plan_id`), and `POST /clusters/{id}/pools` accepts any plan — mix sizes and tiers (General Purpose / CPU-Optimized) in one cluster. Per-pool `node_count` minimum is now 1 (the cluster keeps ≥2 workers overall)
- 🟢 **Added** — **CPU-Optimized worker plans** — `GET /api/v1/k8s/node-plans` returns a `node_class` per plan (`shared` or `cpu_optimized`); CPU-Optimized nodes reserve far more host CPU per vCPU for compute-heavy workloads. Pick one via `worker_plan_id` as usual

- 🟢 **Added** — **List cluster volumes** — `GET /api/v1/k8s/clusters/{cluster_id}/volumes` returns the cluster's PVC-provisioned persistent volumes (size, status, attached node, price); also shown on the cluster's new **Storage** tab in the dashboard
- 🔵 **Updated** — **Persistent volumes support online expansion**: grow a PVC's requested size and the disk and filesystem expand live (no pod restart); billing moves to the new size. Grow only, 10 GB–10 TB

### August 26, 2026

**Kubernetes — API**

- 🟢 **Added** — **Built-in persistent storage** — every new cluster includes the `raff-block` default StorageClass: PersistentVolumeClaims are provisioned as Volumes, attached to the pod's worker automatically, visible in your Volumes list, and billed at \$0.08/GB/month (hourly) for exactly the GB claimed
- 🔴 **Removed** — **Create cluster**: `storage_node_count` and `storage_node_disk_gb` (dedicated Longhorn storage nodes) — any non-zero value is now rejected. Existing clusters with storage nodes keep working unchanged
- 🔵 **Updated** — **Delete cluster** now also deletes the cluster's PVC-backed volumes, so no storage keeps billing after teardown

**Kubernetes — Terraform**

- 🟡 **Deprecated** — `raff_k8s_cluster.storage_node_count` / `storage_node_disk_gb` — leave unset; persistent storage is built in and billed per GB

### August 25, 2026

**Kubernetes — API**

- 🟢 **Added** — **Short-lived kubeconfigs** — `GET /api/v1/k8s/clusters/{id}/kubeconfig?expires_in=<seconds>` issues a kubeconfig that expires on its own (10 minutes to 30 days)
- 🟢 **Added** — **Rotate cluster access** — `POST /api/v1/k8s/clusters/{id}/kubeconfig/rotate` invalidates every previously issued short-lived kubeconfig immediately
- 🔵 **Updated** — **Create cluster**: omitting `version_id` now converges the cluster to the platform-default version (previously it stayed on the base image version)

**Kubernetes — CLI**

- 🟢 **Added** — `raff kubernetes kubeconfig --ttl <duration>` (short-lived kubeconfig) and `raff kubernetes kubeconfig rotate`

**Kubernetes — Terraform**

- 🟢 **Added** — `upgrade_mode`, `maintenance_day`, `maintenance_start` on `raff_k8s_cluster`; changing `version_id` now performs a rolling in-place upgrade (the apply waits for completion) instead of destroying the cluster

- 🔵 **Updated** — **Create cluster** now works on subscription accounts — `POST /api/v1/k8s/clusters` validates the saved payment method, creates the monthly subscription, and charges it automatically (previously `402`; same behavior as VM creation). Terraform and CLI creates work on subscription accounts as a result.
- 🔵 **Updated** — **Scale node pool, Add node pool, Upgrade to HA** now work on subscription clusters via the API — the pro-rated difference is charged to the saved payment method (balance first) and the monthly price is raised (previously `409`). Dashboard flows still use checkout.

### August 24, 2026

**Kubernetes — API**

- 🟢 **Added** — **Version upgrades** — `GET /api/v1/k8s/clusters/{id}/upgrades` (available targets, upgrade state, maintenance settings) and `POST /api/v1/k8s/clusters/{id}/upgrade` (in-place rolling upgrade: control-plane nodes one at a time, workers drained; free)
- 🟢 **Added** — **Maintenance settings** — `PATCH /api/v1/k8s/clusters/{id}/maintenance`: `upgrade_mode` (`manual`, `auto_patch`, `auto_minor`) and a weekly 4-hour maintenance window (`maintenance_day`, `maintenance_start` UTC)
- 🔵 **Updated** — node plans (`ssd_gib`) now state the actual 25 GB node root disk; worker plans are priced on vCPU/RAM, persistent storage is the storage-node product

**Kubernetes — CLI**

- 🟢 **Added** — `raff kubernetes upgrade <cluster-id>` — lists available upgrade targets, or starts a rolling in-place version upgrade with `--version` (`--confirm-single-master` on non-HA clusters)
- 🟢 **Added** — `raff kubernetes maintenance <cluster-id>` — set the upgrade mode (`--mode manual|auto_patch|auto_minor`) and the weekly 4-hour maintenance window (`--day`, `--start` UTC)
- 🟢 **Added** — `raff kubernetes create --version <version-id>` — create a cluster on a specific offered version; the cluster converges to it automatically after provisioning
...
- 🟢 **Added** — `Idempotency-Key` header on `POST /api/v1/k8s/clusters` — retries return the original cluster instead of provisioning twice
- 🟢 **Added** — pagination (`limit`, `offset`) on `GET /api/v1/k8s/clusters`; derived `ready` on clusters and `actual_node_count` on node pools
- 🔴 **Removed** — legacy `POST /api/v1/k8s/clusters/{id}/scale` (use the node-pool scale endpoint) and the internal `vm_id` field on node objects

### August 20, 2026

**Virtual Machines — API**
- 🔵 **Updated** — **Attach floating IP** now supports multiple public IPv4s — `POST /api/v1/vms/{id}/ip` with `ip_id` accepts additional reserved IPv4s, up to 4 per VM. Auto-assign (`type`) is unchanged: available only while the VM has no public IP of that family.
- 🔵 **Updated** — **List VM networks** — `GET /api/v1/vms/{id}/networks` returns `is_primary`, marking the VM's primary public IPv4 interface.
- 🔵 **Updated** — **Detach floating IP** — `DELETE /api/v1/vms/{id}/ip/{nic_id}`: detaching the primary IPv4 while other IPv4s remain promotes the lowest-numbered remaining IPv4 to primary. Clarified billing: reserved IPs keep billing until released; auto-assigned IPs are free and return to the shared pool.

### July 28, 2026

**Managed Databases — API**
- 🟢 **Added** — **Free tier** for PostgreSQL, MySQL and Valkey — plans with `is_free_tier: true` in `GET /api/v1/databases/plans` cost $0 (one per account, fixed storage, no HA or replicas, daily backups with 3-day retention). Free databases pause after 7 idle days: status becomes `suspended` and all data is kept.
- 🟢 **Added** — **Resume a paused database** — `POST /api/v1/databases/{id}/resume` wakes a `suspended` database; it returns through `deploying` and reaches `running` in about a minute.
- 🔵 **Updated** — **`GET /api/v1/databases/plans`** returns `max_connections` (the client connection limit each plan enforces) and `memory_mib` (RAM for sub-GiB plans, e.g. free Valkey = 512). The database `status` enum gains `suspended`.

### July 20, 2026

**Managed Databases — API**
- 🟢 **Added** — **MySQL, ClickHouse, and Apache Kafka engines** — the `engine` field on `POST /api/v1/databases` (and everywhere it appears) now accepts `mysql`, `clickhouse`, and `kafka` alongside `postgres` and `valkey`. Check live availability and versions with `GET /api/v1/databases/engines`.
- 🔵 **Updated** — **`engine` is now a shared `DatabaseEngine` enum** across all database endpoints and schemas; the engine catalog entry schema is named `DatabaseEngineInfo`. No wire-format change — request and response JSON are unchanged.
- 🔵 **Updated** — **`GET /api/v1/databases/{id}/connection`** returns `ca_cert` (PEM CA bundle) for engines with a private certificate authority — currently Kafka. Empty for other engines.

### July 12, 2026

**Virtual Machines — API**
- 🔵 **Updated** — **Backup schedules now report run state** — `GET /api/v1/backup-schedules` and `GET /api/v1/backup-schedules/{id}` return `enabled`, `next_run_at`, `last_run_at`, `last_run_status`, and `consecutive_failures`. Use these to verify backups are running and alert on repeated failures. Existing fields are unchanged.
- 🔵 **Updated** — **Minimum `keep_count` is now 2** on `POST /api/v1/backup-schedules` and `PATCH /api/v1/backup-schedules/{id}`. A retention of 1 does not work reliably with incremental backups. Existing schedules with `keep_count: 1` keep working; the next update must set 2 or more.

### July 8, 2026

**Raff Apps — API**
- 🟢 **Added** — **Raff Apps (PaaS)** — push-to-deploy app services on `/api/v1/apps`. Create web services, private services, workers, cron jobs, and one-off jobs; manage deployments (`POST .../deployments`), rollbacks (`POST .../rollback`), environment variables (`.../env`), custom domains (`.../domains`), scaling (`.../scale`), pause/resume, runtime and build logs, metrics, job runs, usage (`GET /api/v1/apps/usage`), and spend caps (`.../spend-settings`).
- 🟢 **Added** — **List Apps tiers** — `GET /api/v1/apps/tiers` and public `GET /api/v1/public/pricing/apps`. Compute sizes and prices; no authentication required.

**Raff Apps — CLI**
- 🟢 **Added** — `raff apps` command group: `tiers`, `list`, `get`, `create`, `delete`, `deploy`, `rollback`, `scale`, `pause`, `resume`, `logs`, `deployments`, `env` (list/set/delete), `domains` (list/add/delete), and `usage`.

**Raff Apps — Terraform**
- 🟢 **Added** — `raff_app_service` resource, plus `raff_app_service`, `raff_app_services`, and `raff_app_tiers` data sources.

### July 6, 2026

**Catalog — API**
- 🟢 **Added** — **List managed database pricing** — `GET /api/v1/public/pricing/database`. Plans per engine (`postgres`, `valkey`) plus high availability, read replica, and extra storage pricing. Optional `engine` query filter.
- 🟢 **Added** — **List managed Kubernetes pricing** — `GET /api/v1/public/pricing/kubernetes`. Worker node plans plus HA control plane and NVMe storage node pricing.
