...
1. **Create a new key** ([Generate an API key](/products/manage/team-access/quickstart-guides/generate-api-key)) with the same scope as the one you want to rotate. Give it a slightly different name (`Production CI v2`).
...
|---|---|
| `apikey.regenerated` | Who clicked Regenerate, when, which key (by ID and name), the old prefix, the new prefix |
| `apikey.deleted` | Who clicked Delete, when, which key (by ID and name), the prefix at deletion time |
...
<CardGroup cols={3}>
  ...
  <Card title="API keys" icon="lightbulb" href="/products/manage/team-access/concepts/api-keys">
