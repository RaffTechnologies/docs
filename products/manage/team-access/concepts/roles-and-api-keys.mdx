...
This is the model page for permissions on Raff. Read it once and the Roles tab on the Team & Access page makes sense. The two big ideas:
...
<Frame>
  ![Row context menu on a System role showing a single Duplicate action with a copy icon. The Admin and Billing rows are visible behind it.](/images/products/manage/team-access/system-role-duplicate-menu.png)
</Frame>
...
For the full list of System roles and what each one's permissions add up to, see [Permissions matrix](/products/manage/team-access/details/permissions-matrix).
...
A member with **only an account-level role** can navigate the Team & Access page, see the account settings, work with billing if their role allows, and so on — but they get an empty list under Compute, Networking, and every other resource page until you grant them a project role somewhere.
...
- For a fresh invite: pick this role in the [Invite Member](/products/manage/team-access/quickstart-guides/invite-member) dialog
...
This is the centralized role system in action: roles are the single source of truth, and members and keys both reference them. See [Generate an API key](/products/manage/team-access/quickstart-guides/generate-api-key) for the dashboard walkthrough.
