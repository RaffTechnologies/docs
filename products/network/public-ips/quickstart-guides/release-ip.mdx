...
|---|---|
| The IP is still in the pool (no one re-claimed yet) | [Reserve a new IP](/products/network/public-ips/quickstart-guides/reserve-ip) and try the same address — auto-assign cannot target a specific IP, but reserve sometimes can if it's still free. There is **no guarantee** |
