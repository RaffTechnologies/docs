...
```
[VM creation] → IP from pool ─attached──▶ Free
                                          │
                                          ▼
[VM delete OR remove IP]
                                          │
                                          ▼
                                   Returns to pool
...
```
[Reserve IP $3/mo] ─unattached─▶ In your account, billing
        │
        ▼
[Attach to VM] ─attached──▶ In your account, billing, in use
        │
        ▼
[Detach from VM] ─unattached─▶ In your account, billing, ready to re-attach
        │
        ▼
[Release] ─returned to pool─▶ Gone from your account, billing stops
```
