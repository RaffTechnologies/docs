...
You can also reach Edit Rules from the [API Keys list](/products/manage/team-access/quickstart-guides/generate-api-key) — wait, scratch that. Edit Rules opens only from the **Firewall Groups list row** — not from the VM detail page (which only shows attach/detach), and not from the system Default Firewall (which is read-only and shows **View Rules** instead).
...
Every edit produces an `apikey.regenerated`-style audit-log entry — wait, that's API keys. Firewall edits log as `firewall.updated` (or the equivalent) capturing:
