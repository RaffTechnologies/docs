...
These ports are blocked by the system Default Firewall on every public interface, and Raff strips them from any user-rule range that covers them:
...
Everything else inbound is allowed by default. ICMP (ping) is allowed inbound. All outbound is unrestricted by the Default.
