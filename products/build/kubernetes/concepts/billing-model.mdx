...
- **Pay-as-you-go** — every worker/storage node bills per hour at its plan's hourly rate while it exists. Clusters created through the [API](/api-reference/kubernetes/create-cluster) follow the account's billing mode — on a subscription account the API creates and charges the monthly subscription automatically.
...
Only a **manual** scale through the dashboard changes the base: it goes through checkout and your monthly price moves with it.
