...
**The single-master control plane is free.** You pay for worker nodes, \$30/month if you enable the HA control plane, and per-GB for the persistent volumes your PVCs create.
...
A subscription cluster is itemised **one line per node pool**, priced at that pool's own plan — so a cluster mixing plans reads honestly rather than showing a single lump:

| Line | Amount |
| --- | --- |
| Kubernetes node pool "default-pool" — K8s Standard × 2 | \$35.98 |
| Kubernetes node pool "pool-2" — K8s Performance × 1 | \$33.99 |
| Kubernetes — high availability control plane *(HA clusters only)* | \$30.00 |

Usage-based charges arrive on the monthly pay-as-you-go invoice instead, one line per category:

| Line | What it covers |
| --- | --- |
| Kubernetes Storage | every PVC in the account for that month, combined |
| Kubernetes Node Overage | autoscaler nodes above your committed base |

Storage and overage lines are totals per category, not per volume or per node — the per-resource breakdown is in the dashboard's usage view.
...
| Persistent volumes (PVCs) | \$0.10/GB/month, billed hourly for exactly the GB claimed — on both billing modes, never part of the subscription |
