...
|---|---|---|
| **Gateway** | 1 (2 with HA) | The cluster's front door: holds the public IP, NATs outbound traffic, load-balances the Kubernetes API (6443) and ingress traffic (80/443) into the cluster, and serves DNS to the nodes |
...
The cluster's **Nodes** tab shows all of these roles live — control plane, gateway and workers with their internal IPs, per-node CPU/memory and kubelet status:

<Frame>
  ![Nodes tab showing the control plane, gateway and worker nodes with status](/images/products/build/kubernetes/nodes-tab.png)
</Frame>
