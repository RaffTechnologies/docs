...
A **node pool** is a group of identical workers: same node plan, same Kubernetes labels and taints. Each pool has its own plan — mix General Purpose and CPU-Optimized pools in one cluster to match workloads. A cluster has 1+ pools with 1–20 nodes each; the platform keeps a minimum of 2 workers per cluster across all pools.
