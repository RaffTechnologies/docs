...
Node pools group workers, labels and taints. Use separate pools to organize workloads — e.g. a general pool plus a tainted CPU-Optimized pool reserved for databases. Each pool picks its own node plan.
...
<Steps>
  ...
  <Step title="Name it and choose size and count">
    Pool names follow the same rules as cluster names. Pick a node plan — any plan, independent of the other pools — and 1–20 nodes; optionally enable autoscaling with min/max bounds.
  </Step>
  <Step title="Confirm">
