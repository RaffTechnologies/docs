...
Raff Kubernetes gives you a production Kubernetes cluster in about ten minutes: we run and monitor the control plane, you get an admin kubeconfig and deploy with the standard tools. The single-master control plane is free; you pay per worker node. High availability (3 masters) is a paid option at \$30/month. Clusters run RKE2 with Cilium networking inside a dedicated VPC, behind a managed gateway with a public IP.

<Frame>
  ![Kubernetes cluster overview in the Raff dashboard — live node, pod, CPU, memory and control-plane health cards](/images/products/build/kubernetes/hero.png)
</Frame>
