...
## 7. Confirm automatic security updates

Raff Ubuntu and Debian images ship with `unattended-upgrades` enabled for the security pocket. Confirm it is on:
...
On the RHEL family, enable `dnf-automatic`:
...
Raff Ubuntu and Debian images ship with `fail2ban` and the `sshd` jail active (5 failures in 10 minutes → 10 minute ban). Check it and see who has been knocking:

```bash
sudo fail2ban-client status sshd
```

Tune `/etc/fail2ban/jail.local` if you want longer ban times. On the RHEL family install it yourself:
...
- [x] fail2ban running (preinstalled on Raff Ubuntu/Debian images)
